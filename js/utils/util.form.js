...
(function(doc, Util, stringTools) {
    ...
    FormValidator.prototype = {
        ...
        _resolveRegType: function(dom) {
            var regType = dom.dataset.ruleRegtype,
                self = this;
            ...
            if(dom.required || dom.value.length > 0) {
                ...
                if(compatible[regType]) {
                    if(!(compatible[regType](dom.value))) {
                        ...
                        self.toast(dom);
                        self.debug('输入内容未满足正则匹配' + regType);
        ...
        toast: function(dom) {
            ...
            var errMsg = dom.dataset.ruleErrmsg || dom.dataset.ruleMatemsg;

            Util.ejs.ui.toast(errMsg || dom.placeholder || '');
        },

        /**
         * debug模式 输出详细信息 console
         * @param {String} 信息
         */
        debug: function(msg) {
            if(!this.isDebug) {
